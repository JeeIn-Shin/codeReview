...
window.onload = function () {
  if (!accessToken) {
    alert('로그인이 필요합니다.');
    location.href = 'http://localhost:8082/views/login/login.html'; //경로 수정
  } else {
...
function getDataByUsername(username) {
  const url = `http://localhost:8082/community`;
  return fetch('http://localhost:8082/community', {
    method: 'GET',
    mode: 'cors',
    headers: {
...
function postData(data) {
  const url = `http://localhost:8082/community/post?type=create`;
...
function createCommentList(data) {
  ...
  data.forEach(({ comment, date, author }) => {
    ...
    li.className = 'comment byuser comment-author-_smcl_admin even thread-odd thread-alt depth-1';

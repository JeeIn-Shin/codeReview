...
// 첫 째 블록
const first_block2 = 1;

// 현재 블록박스에서 첫번째 블록
let current_block2 = localStorage.getItem("current_block2") || 1;
//number 타입으로 변환
current_block2 = Number(current_block2);
console.log("current_block 값: ", current_block2);
localStorage.setItem("current_block2", current_block2);

//선책한 블록
let select_block2 = localStorage.getItem("select_block2") || 1;
select_block2 = Number(select_block2);
console.log("select_block 값: ", select_block2);
localStorage.setItem("select_block", select_block2);

let totalPage2 = localStorage.getItem("totalPage2");
...
window.onload = function () {
  if (!accessToken) {
    ...
    getDataByUsername(username)
      .then((result) => {
        data2 = result;
        totalPage2 = data2.length;
        console.log("data2: ", data2);
        commentsNumber.innerHTML = data2.length;

        post_data_print(select_block2);
        block_print(current_block2);
      })
      .catch((error) => {
...
function post_data_print(block) {
  ...
  let start = totalPage2 - page_num2 * (block - 1);

  for (let i = start; i >= 1 && i > start - page_num2; i--) {
    createCommentList(data2, i);
  }
}

function block_print(front_block) {
  current_block2 = front_block;
  // 블록의 총 수를 계산한다.
  let total_block =
    totalPage2 % page_num2 == 0
      ? totalPage2 / page_num2
      : totalPage2 / page_num2 + 1;
  ...
  console.log("block_num : ", block_num2);
  for (
    let i = front_block;
    i <= total_block && i < front_block + block_num2;
    i++
  ) {
    // 버튼을 생성한다.
    ...
    button.addEventListener("click", function (event) {
      ...
      localStorage.setItem("current_block2", current_block2);
      console.log("current_block 설정 : ", current_block2);
      localStorage.setItem("select_block2", i);

      select_block2 = localStorage.getItem("select_block2");
      console.log("selct_block 설정 : ", select_block2);
  ...
  // 다음으로 갈 블럭이 없으면
  if (front_block + block_num2 >= total_block) {
...
function before() {
  console.clear();
  block_print(current_block2 - block_num2);
  console.log("이전");
}
...
function getDataByUsername(username) {
  ...
  return fetch("http://localhost:3000/community", {
    ...
    .then((data2) => {
      ...
      return data2.filter((item) => item.username === username);
    })
    .catch((error) => {
      console.error("Error:", error);
    });
...
function createCommentList(data2) {
  const ol = document.getElementById("commentol");

  data2.forEach(({ comment, date, author }) => {
...
function createCommentList(data2, i) {
  const ol = document.getElementById("commentol");
  ...
  const commentText = document.createElement("p");
  commentText.innerText = data2[i - 1].comment;

    ...
    a.fn.modalSteps = function (b) {
        ...
        return (
            c
                .on("show.bs.modal", function () {
                    var l,
                        ...
                        j = d.callbacks["*"],
                        k = d.callbacks[1];
                    d.disableNextButton && i.attr("disabled", "disabled"),
                        h.attr("disabled", "disabled"),
                        e(),
                        f(j),
                        f(k),
                        g.html(d.btnCancelHtml),
                        h.html(d.btnPreviousHtml),
                        i.html(d.btnNextHtml),
                        (m = a("<input>").attr({ type: "hidden", id: "actual-step", value: "1" })),
                        c.find("#actual-step").remove(),
                        c.append(m),
                        (l = 1),
                        (p = l + 1),
                        c.find("[data-step=" + l + "]").removeClass("hide"),
                        i.attr("data-step", p),
                        (n = c.find("[data-step=" + l + "]").data("title")),
                        (o = a("<span>").addClass("label label-success").html(l)),
                        c
                            .find(".js-title-step")
                            .append(o)
                            .append(" " + n),
                        d.getTitleAndStep(m.attr("data-title"), l);
                })
                .on("hidden.bs.modal", function () {
                    var a = c.find("#actual-step"),
                        b = c.find(".js-btn-step[data-orientation=next]");
                    c.find("[data-step]").not(c.find(".js-btn-step")).addClass("hide"),
                        a.not(c.find(".js-btn-step")).remove(),
                        b.attr("data-step", 1).html(d.btnNextHtml),
                        c.find(".js-title-step").html("");
                }),
            c.find(".js-btn-step").on("click", function () {
                ...
                parseInt(e.val()) === m ? h.attr("data-step", "complete").html(d.btnLastStepHtml) : h.attr("data-step", n).html(d.btnNextHtml),
                    d.disableNextButton && h.attr("disabled", "disabled"),
                    c
                        .find("[data-step=" + k + "]")
                        .not(c.find(".js-btn-step"))
                        .addClass("hide"),
                    c
                        .find("[data-step=" + n + "]")
                        .not(c.find(".js-btn-step"))
                        .removeClass("hide"),
                    parseInt(g.attr("data-step")) > 0 ? g.removeAttr("disabled") : g.attr("disabled", "disabled"),
                    "previous" === j && h.removeAttr("disabled"),
                    (o = c.find("[data-step=" + n + "]")),
                    o.attr("data-unlock-continue") && h.removeAttr("disabled"),
                    (p = o.attr("data-title"));

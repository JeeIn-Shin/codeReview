...
async function getData() {
  try {
    const url = `http://localhost:8082/notice`;
    ...
    if (optionValue == 'title') {
      getDataByTitle(scontent.value)
        .then((data1) => {
          ...
          console.log('getDataByTitle : ', data1);
          totalPage = data1.length;
      ...
      getDataByDetails(scontent.value)
        .then((data1) => {
          ...
          console.log('getDataByDetails : ', data1);
          totalPage = data1.length;
          localStorage.setItem('totalPage', totalPage);
          localStorage.setItem('current_block', 1);
          localStorage.setItem('select_block', 1);
...
function deleteDataById(post_id1) {
  try {
    console.log('post_id : ', post_id1);
    const url = `http://localhost:8082/notice`;
    fetch(url, {
      method: 'GET',
      mode: 'cors',
      ...
      .then((notice) => {
        const post = notice.find((n) => n.ID_PK === post_id1);
        return fetch(`http://localhost:8082/notice/?idx=${post.ID_PK}`, {
          method: 'DELETE',
          mode: 'cors',
          headers: {
            'Content-Type': 'application/json',
  ...
  try {
    ...
    return fetch(url, {
      ...
      .then((notice) => {
        ...
        data1 = notice.filter((n) => n.TITLE.toLowerCase().includes(titleLowerCase));
        return data1; // add this line to return the filtered posts
      });
  } catch (error) {
    console.log('error : ', error);
  }
  ...
  try {
    ...
    return fetch(url, {
      ...
      .then((notice) => {
        ...
        data1 = notice.filter((n) => n.DETAILS.toLowerCase().includes(detailsLowerCase));
        return data1; // add this line to return the filtered posts
      });
  } catch (error) {
    console.log('error : ', error);
...
function createPostElement(data1, i) {
  ...
  titleAnchor.textContent = data1[i - 1].TITLE; // 게시글 제목

  titleAnchor.onmouseover = function () {
    accessToken = getCookie('accessToken');
    decoded = parseJwt(accessToken);
    isAdminCheck();
    localStorage.setItem('post_id', data1[i - 1].ID_PK);
    localStorage.setItem('select_block', select_block);
  };
  ...
  authorIcon.textContent = ' ' + data1[i - 1].WRITER; // 게시글 작성자
  ...
  dateIcon.textContent = ' ' + data1[i - 1].DATE; // 게시글 작성일
  ...
  contentParagraph.innerHTML = data1[i - 1].DETAILS; // 게시글 내용
  //css 미적용
  ...
  if (isAdmin) {
    ...
    deleteButton.addEventListener('click', function () {
      if (confirm(data1[i - 1].ID_PK + '번 ' + data1[i - 1].TITLE + ' 정말 삭제하시겠습니까?')) {
        deleteDataById(data1[i - 1].ID_PK);
      } else {
        alert('취소되었습니다.');
      }
    });

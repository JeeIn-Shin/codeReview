...
async function getSelectData(post_id) {
  ...
  try {
    const url = `http://localhost:8082/notice?idx=${post_id}`;
...
function postData(Data) {
  const url = `http://localhost:8082/notice/post`; //?type=create
...
function putData(data) {
  const url = `http://localhost:8082/notice/post?idx=${post_id}`;
  console.log('noticeData: ', data);
  console.log('noticeData.TITLE: ', data.title);
  console.log('noticeData.DETAILS: ', data.details);
  fetch(url, {

...
router.post("/", checkTokens, async (req, res) => {
  try {
    ...
    if (enrollInfo.position === "1") {
      ...
      await matching.registerQueueByReviewee(
        enrollInfo,
        additionalPlan,
        preferData
      );
...
router.post("/update", checkTokens, async (req, res) => {
  ...
  // 1이 reviewee
    ...
    await matching.updatePlanAndPrefer(
      pk.ID_PK,
      revieweeUpdateData,
      (err, res) => {
        try {
  ...
  matching
    ...
    .then(async (revieweesResult) => {
      ...
      while (count <= dataLength) {
          ...
          .then(async (reviewersList) => {
            ...
            for (let index = 0; index < data.length; index++) {
              ...
              for (let key in data[index]) {
                if (key !== "ID_PK") weight += data[index][key];
                else {
                  id = data[index][key];
                }
              }
        ...
        MatchedDateAndTime =
          thingAboutSubQuery.findSameWeekdayAndTimeZoneBetweenMatchedPeople(
            revieweeInfo,
            MatchedReviewerId
          );
        ...
        let matchingData = {
          ...
          reviewerId: root.id,
          datetime: MatchedDateAndTime.weekday.concat(
            MatchedDateAndTime.time[0]
          ),
          state: 0,
        };
        ...
        await Promise.all([
          matching.deleteRevieweeFromQueue(revieweeInfo.ID_PK),
          matching.deleteReviewerFromQueue(root.id),
          matching.setReviewActInfo(matchingData),
        ]).catch((err) => {
          console.log(err);
        });
